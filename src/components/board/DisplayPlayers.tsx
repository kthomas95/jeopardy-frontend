...
export const DisplayPlayers = ({ players }: { players: PlayerViewPlayer[] }) => (
    <div className={"grid grid-flow-col p-2 gap-2 md:h-16 items-end col-span-6 text-white"}>
        {players.map((player) => (
            <div
            ...
            >
                ...
                <div className={flattenStrings(["font-black", player.moneyAmount < 0 ? "text-red-500" : ""])}>
                    ${player.moneyAmount}
                </div>
            </div>
        ))}
